...
let () =
  ...
  let module StringRepresentation = struct
    ...
    let test_to_string_zero () = assert (String.equal "0" (Fr.to_string Fr.zero))

    let test_of_string_with_of_z () =
      List.iter
        (fun x -> assert (Fr.eq (Fr.of_string x) (Fr.of_z (Z.of_string x))))
        test_vectors

    let test_of_string_to_string_consistency () =
      List.iter
        (fun x -> assert (String.equal (Fr.to_string (Fr.of_string x)) x))
        test_vectors

    let get_tests () =
      ( "String representation",
        [ ("one", test_to_string_one);
  ...
  let module ZRepresentation = struct
    ...
    let test_of_z_one () = assert (Fr.eq Fr.one (Fr.of_z Z.one))

    let test_random_of_z_and_to_z () =
      let x = Fr.random () in
      assert (Fr.eq x (Fr.of_z (Fr.to_z x)))

    let test_random_to_z_and_of_z () =
      let x = Z.of_int (Random.int 2_000_000) in
      assert (Z.equal (Fr.to_z (Fr.of_z x)) x)
  ...
  let module TestVector = struct
    let test_inverse () =
      ...
      List.iter
        (fun (e, i) ->
          assert (Fr.eq (Fr.inverse_exn (Fr.of_string e)) (Fr.of_string i)))
        test_vectors ;
      List.iter
        (fun (e, i) ->
          assert (Fr.eq (Fr.inverse_exn (Fr.of_string i)) (Fr.of_string e)))
        test_vectors
    ...
    let test_add () =
      ...
      List.iter
        (fun (e1, e2, expected_result) ->
          assert (
            Fr.eq
              (Fr.add (Fr.of_string e1) (Fr.of_string e2))
              (Fr.of_string expected_result) ))
        test_vectors ;
      List.iter
        (fun (e1, e2, expected_result) ->
          assert (
            Fr.eq
              (Fr.add (Fr.of_string e2) (Fr.of_string e1))
              (Fr.of_string expected_result) ))
        test_vectors
    ...
    let test_mul () =
      ...
      List.iter
        (fun (e1, e2, expected_result) ->
          assert (
            Fr.eq
              (Fr.mul (Fr.of_string e1) (Fr.of_string e2))
              (Fr.of_string expected_result) ))
        test_vectors ;
      List.iter
        (fun (e1, e2, expected_result) ->
          assert (
            Fr.eq
              (Fr.mul (Fr.of_string e2) (Fr.of_string e1))
              (Fr.of_string expected_result) ))
        test_vectors
    ...
    let test_opposite () =
      ...
      List.iter
        (fun (e1, expected_result) ->
          assert (
            Fr.eq (Fr.negate (Fr.of_string e1)) (Fr.of_string expected_result)
          ))
        test_vectors ;
      List.iter
        (fun (e1, expected_result) ->
          assert (
            Fr.eq (Fr.negate (Fr.of_string expected_result)) (Fr.of_string e1)
          ))
        test_vectors
    ...
    let test_pow () =
      ...
      List.iter
        (fun (x, e, expected_result) ->
          assert (
            Fr.eq
              (Fr.pow (Fr.of_string x) (Z.of_string e))
              (Fr.of_string expected_result) ))
        test_vectors
