module MakeUncompressedStubs (M : sig
  ...
  let build_from_components x y =
    ...
    @@ fun_call
         ...
         [| inject 0; inject size_in_bytes ; inject (2 * size_in_bytes) |] in
    if res then (
      let res = Jsoo_lib_rust_wasm.Memory.Buffer.slice M.wasm_memory_buffer 0 size_in_bytes in
...
module MakeCompressedStubs (M : sig
  ...
  let zero () =
    ignore
    @@ fun_call (get M.rust_module "rustc_bls12_381_g1_compressed_zero") [| inject 0 |] ;
    let res = Jsoo_lib_rust_wasm.Memory.Buffer.slice M.wasm_memory_buffer 0 size_in_bytes in
    Jsoo_lib_rust_wasm.Memory.Buffer.to_bytes res

  let one () =
    ignore
    @@ fun_call (get M.rust_module "rustc_bls12_381_g1_compressed_one") [| inject 0 |] ;
    let res = Jsoo_lib_rust_wasm.Memory.Buffer.slice M.wasm_memory_buffer 0 size_in_bytes in
    Jsoo_lib_rust_wasm.Memory.Buffer.to_bytes res

  let random () =
    ignore
    @@ fun_call (get M.rust_module "rustc_bls12_381_g1_compressed_random") [| inject 0 |] ;
    let res = Jsoo_lib_rust_wasm.Memory.Buffer.slice M.wasm_memory_buffer 0 size_in_bytes in
    Jsoo_lib_rust_wasm.Memory.Buffer.to_bytes res
