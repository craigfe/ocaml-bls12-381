...
CAMLprim value ml_librustc_bls12_381_g1_uncompressed_check_bytes(value element) {
  bool res = rustc_bls12_381_g1_uncompressed_check_bytes(Bytes_val(element));
  return Val_bool(res);
}

extern bool rustc_bls12_381_g1_compressed_check_bytes(const unsigned char *element);

CAMLprim value ml_librustc_bls12_381_g1_compressed_check_bytes(value element) {
  bool res = rustc_bls12_381_g1_compressed_check_bytes(Bytes_val(element));
  return Val_bool(res);
}
...
extern bool rustc_bls12_381_g1_build_from_x_and_y(unsigned char *buffer,
                                                  const unsigned char *x,
                                                  const unsigned char *y);

CAMLprim value ml_librustc_bls12_381_g1_build_from_x_and_y(value buffer, value x, value y) {
  CAMLparam3(buffer, x, y);
  bool res = rustc_bls12_381_g1_build_from_x_and_y(Bytes_val(buffer), Bytes_val(x),
                                                   Bytes_val(y));
  CAMLreturn(Val_bool(res));
}
