...
CAMLprim value ml_librustc_bls12_381_fq12_check_bytes(value element) {
  bool res = rustc_bls12_381_fq12_check_bytes(Bytes_val(element));
  return Val_bool(res);
}
...
CAMLprim value ml_librustc_bls12_381_fq12_is_zero(value x) {
  CAMLparam1(x);
  bool res = rustc_bls12_381_fq12_is_zero(Bytes_val(x));
...
CAMLprim value ml_librustc_bls12_381_fq12_is_one(value x) {
  CAMLparam1(x);
  bool res = rustc_bls12_381_fq12_is_one(Bytes_val(x));
  CAMLreturn(Val_bool(res));
}
...
CAMLprim value ml_librustc_bls12_381_fq12_eq(value x, value y) {
  CAMLparam2(x, y);
  bool res = rustc_bls12_381_fq12_eq(Bytes_val(x), Bytes_val(y));
  CAMLreturn(Val_bool(res));
}
...
CAMLprim value ml_librustc_bls12_381_fq12_pow(value buffer, value x, value n) {
  CAMLparam3(buffer, x, n);
  rustc_bls12_381_fq12_pow(Bytes_val(buffer), Bytes_val(x), Bytes_val(n));
  CAMLreturn(Val_unit);
}
