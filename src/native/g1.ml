...
module Uncompressed_Stubs = struct
  ...
  let zero () =
    let bs = empty () in G1_stubs.zero (Ctypes.ocaml_bytes_start bs); bs

  let one () =
    let bs = empty () in G1_stubs.one (Ctypes.ocaml_bytes_start bs); bs

  let random () =
    let bs = empty () in G1_stubs.random (Ctypes.ocaml_bytes_start bs); bs
  ...
  let build_from_components x y =
    let buffer = empty () in
    let res = G1_stubs.build_from_components
      (Ctypes.ocaml_bytes_start buffer)
      (Ctypes.ocaml_bytes_start x)
      (Ctypes.ocaml_bytes_start y) in
...
module Compressed_Stubs = struct
  ...
  let zero () =
    let bs = empty () in G1_stubs.compressed_zero (Ctypes.ocaml_bytes_start bs); bs

  let one () =
    let bs = empty () in G1_stubs.compressed_one (Ctypes.ocaml_bytes_start bs); bs

  let random () =
    let bs = empty () in G1_stubs.compressed_random (Ctypes.ocaml_bytes_start bs); bs
  ...
  let eq x y =
    G1_stubs.compressed_eq (Ctypes.ocaml_bytes_start x) (Ctypes.ocaml_bytes_start y)
